#+PROPERTY: header-args:sh :results output :session llmPyEnv :dir .
#+PROPERTY: header-args:python :async :session llmPyEnv :results output drawer
...
class InstructionDataLoader:
    ...
    def format_instruction(self, data_obj):
        ...
        return formatted, data_obj["role"]=="assistant"
    
    def next_batch(self):
        tokens_read = len(self.x_tokens) - self.current_batch * self.T * self.B
        print(f"tokens_read: {tokens_read}")
        print(f"len(x_tokens): {len(self.x_tokens)}")
        while tokens_read <= self.B * self.T:
            self.current_dialogue += 1
            idx = self.current_dialogue
            if (idx == len(self.text_json[idx])):
                self.reset()
                break
...
manual_args = [
    "--batch_size", "1", "--total_batch_size", "64", "--compile", "1",
    "--output_dir", "./output", "--write_tensors", "1", "--num_iterations",
    "40", "--sample_every", "20",
    "--tensorcores", "1",
    "--instruct", "1"
]
if __name__ == "__main__":
    ...
    # calculate gradient accumulation from the desired total batch size and the
    ...
    print0("peak memory consumption after createing model: "
    ...
    print0("peak memory consumption; model moved to device: "
           f"{torch.cuda.max_memory_allocated() // 1024 // 1024} MiB")
    model.train()
    if args.compile:
        ...
        print0("peak memory consumption; model compiled: "
               f"{torch.cuda.max_memory_allocated() // 1024 // 1024} MiB")
    ...
    if master_process and args.write_tensors and (not args.inference_only):
        ...
        write_model(
            model, os.path.join(
                args.output_dir, f"llama{model_version}_{model_size_str}.bin"),
            dtype="float32")
        write_model(
            model, os.path.join(
                args.output_dir,
    ...
    for step in range(args.num_iterations + 1):
        ...
        if (args.sample_every > 0 \
            ...
            if args.use_hf:
                ...
                prompt_tokens, _ = train_loader.format_instruction(prompts)
