...
MODEL_DICT: Dict[str, LlamaConfig] = {
    ...
    "unsloth/Llama-3.2-1B-Instruct": LLama3_1BConfig,
}
        ...
        for i in range(config.n_layer):
            for name in ['attention.wq', 'attention.wk', 'attention.wv']:
                ...
                if name == 'attention.wq':
                    checkpoint[new_key] = checkpoint.pop(old_key)
                # merge 3 weights into transformer.h.x.attn.c_attn.weight
                else:
...
class DistributedShardedDataLoader:
    ...
    def __init__(self, filename_pattern, B, T, process_rank, num_processes):
        ...
        assert len(self.files) > 0, (
            "did not find any files that match the pattern "
            f"{filename_pattern}")
...
def write_tensors(model_tensors, L, tied, file, dtype):
    ...
    # 1) We want to maintain the ability to finetune just the biases in the
    #    C code and also GPT-2 supported biases and we want to touch as little
    #    code as possible. => We will generate biases of all zeros and write
    #    them here. It's very little data.
    # 2) We want to exactly preserve the GPT-2 code paths, so we can't have
    #    SwiGLU using two separate nn.Linear layers c_fc and c_fc2. We will
    #    merge them into a single c_fc layer. Then later in the C code, we do
    #    pointer arithmetic to recover them fully internal to the SwiGLU
    #    layer.
    # 3) Llama 3 does not use position embeddings table so we have to remove
    #    it. At the same time, and very conveniently, Llama 3 does not share
    #    the output projection weights with the token embeddings table, so we
    #    have to add it. Well instead of removing and adding, we are going to
    #    write the output projection weights into the slot previously used
    #    for the position embeddings table. Everyone is happy, very little
    #    code is changed from GPT-2.
    cleaned_tensors = {}
    ...
    if not tied:
        # (V, C) # <--- hack (3) here!
        write_fun(cleaned_tensors["lm_head.weight"], file) 
    for i in range(L): # (L, C)
        write_fun(cleaned_tensors[f"transformer.h.{i}.ln_1.weight"], file)
    for i in range(L): # (L, C)
        # see hack (1) above for these
        # yes i know this is inefficient and dumb i'm just matching the
        # train_gpt2.py code format
        write_fun(torch.zeros_like(
            cleaned_tensors[f"transformer.h.{i}.ln_1.weight"]), file)
    for i in range(L): # (L, 3C, C)
        write_fun(cleaned_tensors[f"transformer.h.{i}.attn.c_attn.weight"],
                  file)
    for i in range(L): # (L, 3C)
        w = cleaned_tensors[f"transformer.h.{i}.attn.c_attn.weight"]
        write_fun(torch.zeros(w.size(0), dtype=w.dtype), file)
    for i in range(L): # (L, C, C)
        write_fun(cleaned_tensors[f"transformer.h.{i}.attn.c_proj.weight"],
                  file)
    for i in range(L): # (L, C)
    ...
    for i in range(L): # (L, C)
        write_fun(torch.zeros_like(
            cleaned_tensors[f"transformer.h.{i}.ln_2.weight"]), file)
...
def write_model(model, filename, dtype):
    ...
    with open(filename, "wb") as file:
        ...
        write_tensors(params, model.config.n_layer,
                      model.config.tied_embeddings, file, dtype) # params
    print(f"wrote {filename}")

def write_state(model, x, y, logits, loss, filename):
    # the state is used for debugging.
    # it contains information about the input, logits, loss, and the parameter
    # gradients
    ...
    header[4] = 0
    grads = {name: param.grad.cpu() for name, param in (
    ...
    with open(filename, "wb") as file:
        ...
        # gradients
        write_tensors(grads, model.config.n_layer,
                      model.config.tied_embeddings, file, "float32")
    print(f"wrote {filename}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_hf", type=int, default=1,
                        help="use HuggingFace (default) or use Meta's model")
    parser.add_argument(
        "--ckpt_dir", type=str, default=None,
        help="path to llama3 model checkpoint (needed if use_hf=0)")
    parser.add_argument("--tokenizer_path", type=str, default=None,
                        help="path to llama3 tokenizer (needed if use_hf=0)")
    ...
    parser.add_argument("--model", type=str, default="unsloth/Llama-3.2-1B",
                        help="chose the llama model")
    parser.add_argument("--depth", type=int, default=-1,
                        help="load only a subset of the model's layers")
    parser.add_argument(
        "--untie", type=int, default=False,
        help=("Untie token embeddings and LM-head, even if they are tied in"
              " the checkpoint."))
    ...
    parser.add_argument("--grad_clip", type=float, default=1.0,
                        help="maximum gradient magnitude")
    # evaluation
    ...
    # python -> C bridge
    parser.add_argument("--write_tensors", type=int, default=0,
                        help="write tensors to disk")
    parser.add_argument("--instruct", type=int, default=0,
                        help="if 1, trains on instruct dataset")
    ...
    if ddp:
        # use of DDP atm demands CUDA, we set the device appropriately
        # according to rank
        assert torch.cuda.is_available(), (
            "for now i think we need CUDA for DDP")
    ...
    # we need to load LLaMA as bf16 on CUDA
    ...
    if args.use_hf:
        ...
        assert args.ckpt_dir is not None and os.path.exists(args.ckpt_dir),(
            f"llama3 ckpt dir {args.ckpt_dir} does not exist")
        assert (args.tokenizer_path is not None and
                os.path.exists(args.tokenizer_path)), (
                    f"llama3 tokenizer path {args.tokenizer_path} "
                    "does not exist")
        model = LLaMA.from_pretrained_llama3_meta(args.ckpt_dir,
                                                  args.tokenizer_path)
    ...
    # load tokens
    if args.instruct:
        train_loader = InstructionDataLoader(
            args.input_bin, B, T, ddp_rank, ddp_world_size)
    else:
        train_loader = DistributedShardedDataLoader(
            args.input_bin, B, T, ddp_rank, ddp_world_size)
    val_loader = None
    if args.input_val_bin and args.instruct:
        val_loader = InstructionDataLoader(
            args.input_val_bin, B, T, ddp_rank, ddp_world_size)
        val_loader.set_tokenizer(model.tokenizer)
    elif args.input_val_bin:
        val_loader = DistributedShardedDataLoader(
            args.input_val_bin, B, T, ddp_rank, ddp_world_size)
    # -------------------------------------------------------------------------
    # PyTorch -> C bridge: save some weights and state for C to load later as
    # reference
    ...
    for step in range(args.num_iterations + 1):
        ...
        # step <= num_iterations instead of just < num_iterations (one extra
        # due to <=), only to do the validation/sampling one last time, and
        # then we break right here as we're done.
        if last_step:
            break
        ...
        if args.overfit_single_batch:
            train_loader.reset()
        # micro-batch loop where we do gradient accumulation to reach desired
        # total batch size for getting the mean loss (as simple float) over
        ...
        for micro_step in range(grad_accum_steps):
            ...
            if ddp:
                ...
                # that is a context manager that bloats the code, so we just
        ...
        # everything that follows now is just diagnostics, prints, logging,
        # etc.
        ...
        # time and print
        t1 = time.time()
        # the 0th iteration is often an outlier (much slower) => skip logging
        # it
        ...
        print0(
            ...
            f" | norm {norm:.4f} | lr {lr:.2e} | ({(t1-t0)*1000:.2f} ms |"
    ...
    print0("peak memory consumption: "
           f"{torch.cuda.max_memory_allocated() // 1024 // 1024} MiB")
